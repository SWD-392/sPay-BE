		...
		public async Task<SPayResponse<bool>> CreateCustomerAsync(CreateCustomerRequest request)
		{
			using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
			{
				...
				try
				{
					...
					{
						SPayResponseHelper.SetErrorResponse(response, "The customer was areadly exist and banned. Please contact with admin to take action.");
						return response;
					}

					if (userChecked != null && userChecked.Status == (int)UserStatusEnum.Active)
					{
						SPayResponseHelper.SetErrorResponse(response, "The phone number of customer was areadly exist.");
						return response;
					}
